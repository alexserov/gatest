paginate () { # "method" "URL" "items getter"
    ...
    do
        ((page++))
        local value=$(echo $(curl -s -X $1 $2 -H "Accept: application/vnd.github.v3+json" -G -H "Authorization: Bearer $GITHUB_TOKEN" -d per_page=100 -d page=$page) | jq $3)
...
all_requests=$(paginate GET "https://api.github.com/repos/$1/pulls?state=open")
...
for request_ref in $(echo $renovate_request_refs | jq -r .); do
    all_checks=$(paginate GET "https://api.github.com/repos/$1/commits/$request_ref/check-runs" .check_runs)
    ...
    for check_id in $(echo $failed_checks | jq -r .[].check_suite.id); do
        curl -X POST \
            ...
            "https://api.github.com/repos/$1/check-suites/${check_id}/rerequest"
    done
done
